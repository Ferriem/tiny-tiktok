...
func (*FavoriteModel) Favorite(tx *gorm.DB, favorite *Favorite) error {
...
func (*FavoriteModel) DeleteFavorite(tx *gorm.DB, favorite *Favorite) error {
	result := tx.Where("user_id=? AND video_id=?", favorite.UserId, favorite.VideoId).Delete(&favorite)
...
func (*FavoriteModel) FavoriteList(userId int64) ([]int64, error) {
	var videoIds []int64
