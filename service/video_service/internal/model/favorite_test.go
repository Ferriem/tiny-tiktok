...
func TestAddFavorite(t *testing.T) {
	InitDB()
	err := GetFavoriteInstance().AddFavorite(&Favorite{
		UserId:  1,
...
func TestDeleteFavorite(t *testing.T) {
	InitDB()
	err := GetFavoriteInstance().DeleteFavorite(&Favorite{
		UserId:  1,
		VideoId: 446461502582784,
	})
	if err != nil {
...
func TestGetFavoriteList(t *testing.T) {
	InitDB()
	var videoId []int64
	videoId, _ = GetFavoriteInstance().GetFavoriteList(1)
	fmt.Println(videoId)
}
...
func TestFavoriteUserList(t *testing.T) {
	InitDB()
	var userId []int64
	userId, _ = GetFavoriteInstance().FavoriteUserList(1)
	fmt.Println(userId)
}
