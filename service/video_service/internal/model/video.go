...
func (*VideoModel) PlusFavoriteCount(videoId int64) error {
	result := DB.Exec("UPDATE video SET favorite_count = favorite_count + 1 WHERE id=?", videoId)
	if result.Error != nil {
		return result.Error
	}
	return nil
...
func (*VideoModel) MinusFavoriteCount(videoId int64) error {
	result := DB.Exec("UPDATE video SET favorite_count = favorite_count - 1 WHERE id=?", videoId)
	if result.Error != nil {
		return result.Error
	}
...
func (*VideoModel) PlusCommentCount(videoId int64) error {
	result := DB.Exec("UPDATE video SET comment_count = comment_count + 1 WHERE id=?", videoId)
	if result.Error != nil {
		return result.Error
	}
	return nil
...
func (*VideoModel) MinusCommentCount(videoId int64) error {
	result := DB.Exec("UPDATE video SET comment_count = comment_count - 1 WHERE id=?", videoId)
	if result.Error != nil {
		return result.Error
	}
