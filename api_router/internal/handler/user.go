...
func GetUserInfo(userIds []int64, ctx *gin.Context) (userInfos []response.User) {
	...
	for index := range userIds {
		userInfos = append(userInfos, BuildUser(userResp.Users[index], followInfoResp.FollowInfo[index]))
	}
	return userInfos
}

func BuildUser(user *proto.User, follow *proto.FollowInfo) response.User {
